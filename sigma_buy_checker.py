...
def build_table(df_map, prev_price, title):
    ...
    for t, df in df_map.items():
        closes   = df['Close']
        # 60거래일 수익률 시리즈 → 일별 σ, 연환산은 생략 (전일 종가 기준으로 밴드 계산)
        ret60    = closes.pct_change(60).dropna()
        sigma60  = float(ret60.std() * 100)    # σ₆₀ %
        # 기준가: 252일 단순이동평균
...
if __name__=="__main__":
    ...
    df1 = {t: yf.download(t, period="365d", progress=False)[["Close"]].dropna()
           for t in tickers}
    title1, tab1 = build_table(df1, prev_price,
        "@최근1년기준(전일종가 기준)")
    ...
    df2 = {t: yf.download(
           ...
           for t in tickers}
    title2, tab2 = build_table(df2, prev_price,
        "@전월동기→1년기준(전일종가 기준)")
